...
export function calculateWeightLossWeekly(
  ...
  const D_min = new Date(
    firstRecord.date.getFullYear(),
    firstRecord.date.getMonth(),
    firstRecord.date.getDate()
  );
  const D_max = new Date(
    lastRecord.date.getFullYear(),
    lastRecord.date.getMonth(),
    lastRecord.date.getDate()
  );
    ...
    if (exactLog) {
      ...
      if (prevLog && nextLog) {
        const prevDate = new Date(
          prevLog.date.getFullYear(),
          prevLog.date.getMonth(),
          prevLog.date.getDate()
        );
        const nextDate = new Date(
          nextLog.date.getFullYear(),
          nextLog.date.getMonth(),
          nextLog.date.getDate()
        );
        const totalDays = Math.round(
          (nextDate.getTime() - prevDate.getTime()) / (1000 * 60 * 60 * 24)
        );
        const daysFromPrev = Math.round(
          (current.getTime() - prevDate.getTime()) / (1000 * 60 * 60 * 24)
        );

        const weight =
          prevLog.minWeight + (nextLog.minWeight - prevLog.minWeight) * (daysFromPrev / totalDays);
